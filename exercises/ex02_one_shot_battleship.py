...
while (row_guess > 4) or (row_guess < 1):
    row_guess: int = int(input(f"The grid is only {grid_size} by {grid_size}. Try again: "))
# Guessing a Column
column_guess: int = int(input("Guess a column: "))
while (column_guess > 4) or (column_guess < 1):
    column_guess: int = int(input(f"The grid is only {grid_size} by {grid_size}. Try again: "))
