...
def input_guess(grid_size: int, row_column_guess: str) -> int:
    ...
    row_column_guess: int = int(input(f"Guess a {row_column_guess}: "))
    #Gives a "Try Again" message if guess is outside grid size
    while (int(row_column_guess) > grid_size) or (int(row_column_guess) < 1):
        row_column_guess = int(input(f"The grid is only {grid_size} by {grid_size}. Try again: "))
    return row_column_guess
